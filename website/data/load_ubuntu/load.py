...
class LoadData():
    ...
    def main(self,loc_c,prod_c,list_c,listU_c,mode,load_listing_via_s3,erase_listings,from_date_s="",to_date_s=""):
        ...
        info_by_product = list(listU_c.aggregate(pipeline=pipe))
        productList = []
        for product in info_by_product:
        ...
        info_by_location = list(listU_c.aggregate(pipeline=pipe))
        locationList = []
        for location in info_by_location:
